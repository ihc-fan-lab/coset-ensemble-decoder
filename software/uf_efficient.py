...
def cal_manhattan_distance(point1, point2, code_structure):
    """计算两点之间的曼哈顿距离
    Args:
    ...
    if code_structure.periodic:
        # 对于周期性边界，计算最短距离
        dx = min(abs(x1 - x2), code_structure.L - abs(x1 - x2))
        dy = min(abs(y1 - y2), code_structure.L - abs(y1 - y2))
        dz = abs(z1 - z2)  # z方向通常不周期性
    else:
        dx = abs(x1 - x2)
        dy = abs(y1 - y2)
        dz = abs(z1 - z2)
...
def union(x, y, clusters, code_structure):
    ...
    # 合并两个cluster已有的syndrome点连接信息
    # if not isinstance(clusters[x][6], set):
    #     clusters[x][6] = set()
    if isinstance(clusters[y][5], set):
        clusters[x][5].update(clusters[y][5])
    
    # 计算两个cluster之间所有syndrome点的连接关系
    if clusters[x][4] and clusters[y][4]:
        # 计算所有点对之间的距离，找到最小距离
        min_distance = float('inf')
        min_distance_pairs = []  # 存储所有距离为最小值的点对
        
        for point1 in clusters[x][4]:
            for point2 in clusters[y][4]:
                distance = cal_manhattan_distance(point1, point2, code_structure)
                if distance < min_distance:
                    min_distance = distance
                    min_distance_pairs = [(point1, point2)]
                elif distance == min_distance:
                    min_distance_pairs.append((point1, point2))

        
        # 直接保存最小距离的坐标对
        for point1, point2 in min_distance_pairs:
            # 确保坐标对是有序的（较小的坐标在前）
            if point1 < point2:
                pair = (point1, point2)
            else:
                pair = (point2, point1)
            clusters[x][5].add(pair)

    # 合并初始syndrome点集合
    clusters[x][4].update(clusters[y][4])
...
def union_find_decoder(syndrome, code_structure, random_seed=None, error_type='x'):
    ...
    for vertex in clusters:
        root = find(vertex, clusters, boundaries, code_structure)
    ...
    # 收集每个cluster的syndrome点连接信息
    for root in cluster_roots:
        for connection in clusters[root][5]:  # 如果有连接信息
            cluster_syndrome_connections.append(connection)
...
def spanning_tree_dict(graph, visited, tree, vertex, code_structure):
    ...
    # 随机化邻居节点的访问顺序
    neighbours = list(graph[vertex])  # 将 set 转换为 list
...
def peeling(code_structure, F, syndrome_dict, num_faults, error_type='x'):
    ...
    # 计算单个解的权重
    # weight = len(A_reconstruct)  # 这里简单地用边的数量作为权重，你可以根据需要修改权重计算方式
    return correction, weight




def uf_efficient(syndrome, code_structure, error_type='x', list_size = 1):
    # 保存当前随机状态，避免随机种子污染
